resource "null_resource" "configure_adm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo ssh-keyscan github.com >> /tmp/known_hosts && sudo chown root:root /tmp/known_hosts && sudo mv /tmp/known_hosts /root/.ssh/known_hosts && sudo git clone git@github.com:mkay1905/salt-cloudmedia-repo.git /srv/salt-cloudmedia-repo",
